...
$oV->selfurl = 'qtfm_import_adm.php';
$oV->selfname = L('Import_Admin');
$oV->exiturl = $oV->selfurl;
$oV->exitname = $oV->selfname;
$oV->selfversion = L('Import_Version').' 4.0';
...
if ( isset($_POST['ok']) )
{
  ...
  if ( empty($error) )
  {
    ...
    $oHtml->pageMsg('', '<p class="small">'.L('Item',$arrCounts['topic']).'<br>'.L('Reply',$arrCounts['reply']).'</p><br>'.L('Import_S_import'), 'admin');
  }
}
...
echo '<form method="post" action="'.$oV->selfurl.'" enctype="multipart/form-data">
