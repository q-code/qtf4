...
$oV->selfurl = 'qtfm_rss_adm.php';
$oV->selfname = $L['rss']['Admin'];
$oV->selfversion = $L['Version'].' 4.0';
...
echo '
<form method="post" action="'.$oV->selfurl.'">
<h2 class="subtitle">'.L('Status').'</h2>
<table class="t-conf">
<tr>
<th>'.L('Status').'</th>
<td class="flex">
<p><span style="display:inline-block;width:15px;background-color:'.( $_SESSION[QT]['m_rss'] ? 'green' : 'red').';border-radius:3px">&nbsp;</span>&nbsp;'.L(($_SESSION[QT]['m_rss'] ? 'On' : 'Off').'_line').'</p>
...
echo '<p style="margin:0 0 5px 0;text-align:center"><button type="submit" name="ok" value="ok">'.L('Save').'</button></p>
</form>
<br>
';
...
if ( count($arrRss)>0 )
{
  echo '<h2 class="subtitle">'.L('Preview').'</h2>'.PHP_EOL;
  ...
  echo '</table></div>';
  echo '<p class="minor"><i class="fa fa-info-circle"></i> The feeds remain accessible when the module is off-line.</p>';
}
