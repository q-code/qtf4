...
if ( isset($_POST['ok']) && $pan>0 )
{
  ...
  if ( $pan==2 ) try {
    ...
    else
    {
      ...
      $test_login.='<br><small style="color:red">&gt; '.$oH->error.'<br>&gt; Possible cause: '.($intEntries==0 ? 'the username does not exists (or is not in the group specied by the login DN)' : 'username exists but the password is invalid') .'</span>';
    }
...
{
echo '<p>Current authority is <span class="bold italic">Internal authority (default)</span>.<br>When module is on-line, change the authority in the page <a href="qtf_adm_secu.php">'.L('Board_security').'</a>.</p><br>';
}
...
<small>Host and port. Ex.: </small><small style="color:#4444ff">ldap://localhost:10389</small></td>
</tr>
<tr>
<th>Login DN</th>
<td><input type="text" id="m_ldap_login_dn" name="m_ldap_login_dn" size="30" maxlength="64" value="'.$_SESSION[QT]['m_ldap_login_dn'].'"/><br>
<small>Use $username as placeholder. Ex.: </span><small style="color:#4444ff">cn=$username,ou=users,o=mycompany</small></td>
</tr>
</table>
';
...
<small>dn or rdn (search basis)</small></td>
</tr>
<tr>
<th>Search filter</th>
<td><input type="text" id="m_ldap_s_filter" name="m_ldap_s_filter" size="30" maxlength="64" value="'.$_SESSION[QT]['m_ldap_s_filter'].'"/><br>
<small>Use $username as placeholder. Ex.: </small><small style="color:#4444ff">(cn=$username)</small><small> allows searching the username specified in the login panel</small></td>
</tr>
<tr>
...
<small>At least the mail is recommended. This is usefull when a user performs his very first login (the application will create a new profile with the same e-mail as in ldap).</small></td>
</tr>
</table>
';
